        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if isinstance(batch_y, list) and isinstance(outputs, list):
                    ...
                    pred = outputs[0].detach().cpu() * std_X + mean_X
                    true = batch_y[0].detach().cpu() * std_X + mean_X
                    ###############################################################
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if isinstance(batch_y, list) and isinstance(outputs, list):
                    ##############################################################
                    mean_X, std_X = self.Data.scaler.mean_, self.Data.scaler.scale_
                    outputs = outputs[0].detach().cpu().numpy() * std_X + mean_X
                    batch_y = batch_y[0].detach().cpu().numpy() * std_X + mean_X
                    ##############################################################
                else:
                    ##############################################################
                    mean_X, std_X = self.Data.scaler.mean_, self.Data.scaler.scale_
